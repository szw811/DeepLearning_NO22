...
from model2 import MyModle_GoogLeNet
from model3 import MyModle_VGG
...
def main():
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-4)# 定义一个优化器
    print(model)# 打印出网络结构
    ...
    for epoch in range(30):
        ...
        for batchidx, (x, label) in enumerate(tqdm(cifar_train)):# enumerate可以生成类似元组 既得到元素又得到索引
            ...
            logits_hair, logits_hair_color, logits_gender = model(x)# 调用Lenet5的forward方法

            # logits: [b, 10]
            # label: [b]
            # loss: tensor scalar 长度为0的标量
            loss_hair = criteon(logits_hair, label[:, 0].long())# 调用了criteon的forward方法
            loss_hair_color = criteon2(logits_hair_color, label[:, 1].long())
            loss_gender = criteon(logits_gender, label[:, 2].long())
            loss = loss_hair_color + 0.25 * loss_hair + 0.3 * loss_gender
        ...
        with torch.no_grad():# 告诉pytorch不需要后向传播
            ...
            for x, label in tqdm(cifar_test):
                ...
                # logits = model(x)
                logits_hair, logits_hair_color, logits_gender = model(x)
                # [b]
            ...
            print("epoch:", epoch, " acc1:", test_acc1, " acc2:", test_acc2, " acc3:", test_acc3)
        ...
        if test_acc2 > best_acc2:
            ...
            best_acc3 = test_acc3
            torch.save(model.state_dict(), model_path)
            print('saving model with acc1 ', round(best_acc1, 3), ' acc2', round(best_acc2, 3), ' acc3', round(best_acc3, 3))

    print('best acc1:', round(best_acc1, 3), ' best acc2:', round(best_acc2, 3), ' acc3', round(best_acc3, 3))
